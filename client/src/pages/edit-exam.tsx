...
export default function EditExam() {
  ...
  useEffect(() => {
    // Load the most recent exam for editing
    const exams = Storage.getExams();
    if (exams.length > 0) {
      const latestExam = exams[exams.length - 1];
      setExam(latestExam);
      setExamTitle(latestExam.title);
      setExamTimer(latestExam.timer);
      setStudentPassword(latestExam.studentPassword);
      setGradePassword(latestExam.gradePassword);
      setQuestions(latestExam.questions);
    }
  }, []);
  ...
  const handleSaveExam = () => {
    ...
    const updatedExam: Exam = {
      id: exam?.id || ExamUtils.generateId(),
      title: examTitle,
      timer: examTimer,
      studentPassword,
      gradePassword,
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Modify your examination</p>
              </div>
            </div>
